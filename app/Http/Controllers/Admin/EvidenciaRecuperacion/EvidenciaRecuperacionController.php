...
namespace App\Http\Controllers\Admin\EvidenciaRecuperacion;
...
class EvidenciaRecuperacionController extends Controller
{
    public function actionEvidenciaRecuperacion(Request $request)
    {
        ...
        // Obtener evidencias con filtro opcional
        $evidenciasQuery = EvidenciaRecuperacion::with(['sesionRecuperacion.planRecuperacion.permiso.docente']);
        ...
        // Obtener todas las sesiones para el selector
        $sesiones = SesionRecuperacion::with(['planRecuperacion.permiso.docente'])->get();

        // Calcular estadísticas por tipo (filtradas si hay sesión seleccionada)
        ...
        if ($sesionIdFilter) {
            $sesionSeleccionada = SesionRecuperacion::with(['planRecuperacion.permiso.docente'])
                ->find($sesionIdFilter);
        }

        return view('admin/evidencia/evidencia_recuperacion', compact(
            'evidencias',
            'sesiones',
            'totalActas',
            'totalAsistencias',
