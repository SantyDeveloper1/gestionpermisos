<?php
namespace App\Http\Controllers\Docente\SesionRecuperacion;
...
class SesionRecuperacionController extends Controller
{
    public function actionSesionRecuperacion(\Illuminate\Http\Request $request)
    {
        ...
        // Obtener planes de recuperación activos (APROBADO o PRESENTADO) SOLO del docente autenticado
        $planesActivos = PlanRecuperacion::with(['permiso.docente.user', 'permiso.tipoPermiso'])
        ...
        // Obtener todos los planes del docente para el filtro
        $planes = PlanRecuperacion::with(['permiso.docente.user'])
            ->whereHas('permiso', function ($query) use ($docente) {
                $query->where('id_docente', $docente->idDocente);
            })
            ->orderBy('fecha_presentacion', 'desc')
        ...
        // Obtener sesiones de ejecución con filtro opcional - SOLO del docente autenticado
        $sesionesQuery = SesionRecuperacion::with(['planRecuperacion.permiso.docente.user', 'planRecuperacion.permiso.tipoPermiso', 'asignatura'])
        ...
        // Sesiones activas (EN_PROGRESO o PROGRAMADA) - SOLO del docente autenticado
        ...
        // Horas recuperadas hoy - SOLO del docente autenticado
        ...
        // Obtener plan seleccionado si existe el filtro - verificar que pertenezca al docente
        ...
        return view('docente/sesion_recuperacion/sesion_recuperacion', compact(
            'sesionesActivas',
            'horasRecuperadasHoy',
            'planes',
            'sesionesEjecucion',
