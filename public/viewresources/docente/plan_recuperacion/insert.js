...
function buscarAsignatura() {
    ...
    $.ajax({
        url: _urlBase + '/docente/asignatura/buscar',
        type: 'GET',
        data: { codigo: codigo },
...
function registrarPlanYSesion() {
    ...
    $.ajax({
        url: form.attr('action') || _urlBase + '/docente/plan_recuperacion/insert',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
...
function obtenerProgresoRecuperacion(idPermiso, horasTotales) {
    $.ajax({
        url: _urlBase + '/docente/plan_recuperacion/progreso/' + idPermiso,
        type: 'GET',
        success: function(response) {
            if (response.success) {
