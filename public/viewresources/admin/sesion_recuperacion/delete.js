...
// Función global para eliminar sesión de recuperación
window.eliminarSesion = function(idSesion) {
    swal({
        title: 'Confirmar Eliminación',
        text: '¿Está seguro de que desea eliminar esta sesión de recuperación? Esta acción es irreversible y eliminará permanentemente la sesión y sus datos.',
        icon: 'warning',
        buttons: {
            cancel: {
                text: 'Cancelar',
        ...
        let $row = $(`#sesionRow${idSesion}`);
        if (!$row.length) {
            // Si no tiene ID específico, buscar por el botón
            const $btn = $(`.btn-delete[onclick*="${idSesion}"]`);
            $row = $btn.closest('tr');
        }
        ...
        $.ajax({
            url: `${_urlBase}/admin/sesion_recuperacion/delete/${idSesion}`,
            type: 'POST',
            data: { _method: 'DELETE' },
            success: function(response) {
                ...
                if (ok) {
                    ...
                    try {
                        ...
                        // No es una DataTable o no está inicializada
                        table = null;
                    }
                    ...
                    new PNotify({
                        ...
                        text: response.message || 'La sesión ha sido eliminada exitosamente del sistema.',
                        type: 'success'
                    });
                } else {
                    new PNotify({
            ...
            error: function(xhr) {
                ...
                if (xhr.status === 422) {
                    ...
                    errorMsg = xhr.responseJSON?.message || 'Sesión no encontrada en el sistema.';
                ...
                $deleteBtn.prop('disabled', false).html('<i class="fas fa-trash"></i>');
                console.error('Error al eliminar sesión:', xhr);
            }
        });
    });
};